...
export function AdminSyncLogPage() {
  ...
  useEffect(() => {
    ...
    const onFail = (err: unknown) => {
      failCount++
      if (!cancelled && failCount >= 2) {
        addToast({ title: 'Failed to load sync logs', description: getErrorMessage(err), variant: 'destructive' })
      }
    }
    ...
    Promise.all([
      adminApi.getSyncLogs()
        ...
        .catch(onFail),
      adminApi.getPurchaseSyncLogs()
        .then(({ data }) => { if (!cancelled) setPurchaseLogs(data.items) })
        .catch(onFail),
